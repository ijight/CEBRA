...
if os.path.exists(processed_data_path) and os.path.exists(processed_labels_path):
    ...
    for file_name in file_list:
        ...
        for i, (start, end) in enumerate(zip(starts, ends)):
            ...
            trial_orientations.append(torch.LongTensor([orientations[i]] * (end - start)))  # Repeat orientation
            trial_temporal_frequencies.append(torch.LongTensor([temporal_frequencies[i]] * (end - start)))  # Repeat frequency
        ...
        if session_id not in holdout_session_ids:
            ...
            labels.append(temporal_frequencies_behavior)
        else:
            # Holdout sessions are split into train and test for later evaluation

            split_index = int(neural_data_tensor.shape[0] * 0.8)  # 80-20 split
            train_data = neural_data_tensor[:split_index]
            test_data = neural_data_tensor[split_index:]
            train_label = temporal_frequencies_behavior[:split_index]
            test_label = temporal_frequencies_behavior[split_index:]
